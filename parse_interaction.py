...
reps = 3
lipid_df = pd.DataFrame(index = np.arange(num_residues))
...
for lipid in lipid_names:
	...
	print(rep)
        ...
        for resid in range(num_residues): #loop through each residue
            ...
            ## add to pandas DF!! This will contain all 3 replicas, we will put them together into 1 dataframe, and then add it to the final dataframe
        lipid_df['%s.%s'%(lipid,rep)] = pd.Series(resid_dict)
        
lipid_df.to_pickle('%s.interaction_counts.pkl' %filename)
